...
void MX_EEPROM_RW_Process()
{
  ...
  if (IT_receive == 1U)
  {
    ...
    {
      HAL_UART_Receive_Until_CR(&hcom_uart[COM1], (uint8_t *)aRxBuffer, 30, 0xffffff);
    }
...
void M95P32_WriteProtectedArea()
{
  ...
  /* Write Disable called explicitly */
  /* Write Enable bit in status register will not reset automatically, previous write not performed. Chip protected. */
  PGEEZ1_M95P32_WriteDisable(PGEEZ1_M95P32_0);
...
void PageProgram_with_BufferLoad(uint32_t Instance, uint8_t *pData, uint32_t start_prog_add, uint32_t nb_byte)
{
  ...
  while (remainingSize > 0)
  {
    ...
    } while ((volatile_reg_val & 0x01) != 0);  /* Poll until BUFLD bit is 0 */
